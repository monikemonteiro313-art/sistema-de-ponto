...
import { ThemeColors, User, PontosGlobal, Jornada, EmpresaConfig, PeriodoFerias, DiaPontos } from "../types";
...
import { getJornada, JORNADAS_PREDEFINIDAS } from "../data/mockData";
...
<table>
  ...
  function alterarInsalubridade(userId: number, valor: 0 | 20 | 40) {
    ...
    showToast(`Insalubridade de ${u.nome} atualizada para ${txtNovo}`);
  }
  ...
  function alternarApenasSomarHoras(userId: number, status: boolean) {
    ...
    setUsers(prev =>
      prev.map(x =>
        x.id === userId
          ? { ...x, apenasSomarHoras: status }
          : x
      )
    );
    if (u) {
      onAddLog(
        "Alterou Jornada Somatória",
        `${u.nome} (${u.matricula})`,
        `Jornada de somatória de horas totais: ${status ? "Sim" : "Não"}. Responsável: ${currentUser.nome}`
      );
    }
    showToast(status ? "Definido como jornada de somatória de horas" : "Retornado para jornada normal");
  }
          ...
          <div style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: 14, padding: "20px 24px", marginBottom: 20 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16, marginBottom: 16 }}>
              ...
              <label style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", background: t.surfaceAlt, border: `1.5px solid ${activeUser.apenasSomarHoras ? t.accent : t.border}`, borderRadius: 10, cursor: "pointer", userSelect: "none" }}>
                <input
                  type="checkbox"
                  checked={!!activeUser.apenasSomarHoras}
                  onChange={e => alternarApenasSomarHoras(activeUser.id, e.target.checked)}
                  style={{ width: 17, height: 17, cursor: "pointer" }}
                />
                <div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: t.text }}>📊 Somatória de Horas Totais</div>
                  <div style={{ fontSize: 11, color: t.textSub }}>Apenas soma horas trabalhadas (não gera faltas ou atrasos)</div>
                </div>
              </label>
            </div>
            ...
            <div>
              ...
              <div style={{ background: t.surface, border: `1.5px solid ${t.border}`, borderRadius: 14, padding: "18px", marginBottom: 16 }}>
                ...
                <GradeCal
                  ...
                  t={t}
                  onDayClick={dayKey => setModalLancamento({ userId: activeUser.id, dayKey })}
                />
              </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }} title="Carga horária mínima trabalhada recomendada para concessão do vale-alimentação diário">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  // parse base 10 to ignore leading zeros
